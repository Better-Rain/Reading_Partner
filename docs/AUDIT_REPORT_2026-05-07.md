...
| R4 | AI 流式请求缺少显式超时、取消和 SSE 容错 | 大幅缓解 | 已加入 120 秒超时、`AbortController`、更宽容的 SSE `data:` 解析和 JSON 解析保护；renderer 已提供运行中取消按钮，主进程可按 `requestId` abort 正在运行的 AI 请求。 | 仍缺少更细的 provider 错误分类。 |
...
4. 给 AI 请求增加更细的 provider 错误分类，并在 UI 中区分鉴权、限流、网络和模型错误。

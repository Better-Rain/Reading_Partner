...
| --- | --- | --- | --- | --- |
| R1 | `src/renderer/src/App.tsx` 过大，前端职责集中 | 大幅缓解，继续优化 | 已拆出 AI 面板、笔记面板、搜索/设置/词汇面板、Markdown 渲染、批注浮层、PDF 搜索高亮、阅读器工具栏、阅读器表面、窗口栏、文档库、选区工具条、快捷键/平移/搜索定位/视口重置 hooks、PDF 文本索引 hook、批注撤销 hook、AI 操作记录 hook、AI 会话状态 hook、词汇本操作 hook。`App.tsx` 已降至约 1081 行。 | 继续拆 AI 流式完成处理、批注 CRUD 等业务 hooks。 |

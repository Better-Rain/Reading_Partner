...
截至本次更新，多个风险已完成或部分缓解：`App.tsx` 已从审计时约 3700+ 行下降到约 1290 行；AI SSE 容错、KeyStore 容错、StarDict 文件句柄复用、数据库合并异步原子写入、PDF 文本抽取进度/取消、安装路径文档均已处理。仍建议继续拆业务 hooks，并对 PDF 索引和数据库持久化做更深层的后台化/增量化改造。
